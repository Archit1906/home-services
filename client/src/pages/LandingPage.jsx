...
export default function LandingPage() {
  ...
  // AI Match Simulator Slider States
  const [sliderBudget, setSliderBudget] = useState(15000);
  const [sliderDistance, setSliderDistance] = useState(3);
  const [sliderExperience, setSliderExperience] = useState(4);
  const [sliderAvailability, setSliderAvailability] = useState(4); // 4 = Full Time, 3 = Flexible, 2 = Part Time, 1 = Any Time

  // Dynamic values calculated from matching slider controls
  const [matchScore, setMatchScore] = useState(92);
  const [locPct, setLocPct] = useState(94);
  const [budPct, setBudPct] = useState(90);
  const [expPct, setExpPct] = useState(88);
  const [availPct, setAvailPct] = useState(96);

  // Re-calculate the compatibility matches when inputs shift
  useEffect(() => {
    // Distance match percentage
    const locMatch = Math.min(99, Math.max(70, Math.round(100 - (sliderDistance - 1) * 3)));
    
    // Budget match percentage (ideal around 12000-15000 range in seed data)
    const budgetDiff = Math.abs(sliderBudget - 15000);
    const budMatch = Math.min(99, Math.max(68, Math.round(100 - (budgetDiff / 140))));
    
    // Experience match percentage (more experience = higher)
    const expMatch = Math.min(99, Math.max(65, Math.round(75 + sliderExperience * 2.3)));
    
    // Availability match percentage (full-time = highest)
    const availMatch = Math.min(99, Math.max(72, Math.round(80 + sliderAvailability * 4.5)));

    setLocPct(locMatch);
    setBudPct(budMatch);
    setExpPct(expMatch);
    setAvailPct(availMatch);

    // Compute composite match score
    const avg = Math.round((locMatch + budMatch + expMatch + availMatch) / 4);
    setMatchScore(Math.min(99, Math.max(70, avg)));
  }, [sliderBudget, sliderDistance, sliderExperience, sliderAvailability]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-text-primary dark:text-text-darkPrimary font-body">
      ...
      {/* 5. LIVE JOB FEED & INTERACTIVE MATCH DEMO SIMULATOR */}
      <section className="py-20 bg-white dark:bg-slate-900 border-y border-border/20 px-6 lg:px-16">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
          
          {/* Left Panel: Live Job Feed (5 cols) */}
          <div className="lg:col-span-5 space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-display font-black tracking-tight">Live Job Feed</h3>
                <span className="inline-flex items-center px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 text-[10px] font-extrabold rounded-full animate-pulse uppercase tracking-wider">
                  &bull; Live
                </span>
              </div>
              <button onClick={() => navigate('/auth')} className="text-xs font-bold text-primary hover:underline">See all jobs &rarr;</button>
            </div>

            {/* Simulated Live Jobs List */}
            <div className="space-y-4">
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-border/10 rounded-2xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 bg-amber-100 text-amber-700 rounded-full flex items-center justify-center">
                    <ChefHat className="h-5 w-5" />
                  </div>
                  <div>
                    <h4 className="font-bold text-xs">Need Cook for full-time</h4>
                    <span className="text-[10px] text-text-secondary font-semibold uppercase tracking-wide">Andheri West, Mumbai</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-black text-text-primary">₹12,000 / month</div>
                  <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md">8 applicants</span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-border/10 rounded-2xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs">Electrician for Wiring</h4>
                    <span className="text-[10px] text-text-secondary font-semibold uppercase tracking-wide">Borivali, Mumbai</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-black text-text-primary">₹900 - ₹1,300</div>
                  <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md">4 applicants</span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-border/10 rounded-2xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs">Experienced Maid Needed</h4>
                    <span className="text-[10px] text-text-secondary font-semibold uppercase tracking-wide">Powai, Mumbai</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-black text-text-primary">₹10,300 / month</div>
                  <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md">6 applicants</span>
                </div>
              </div>
            </div>

            {/* Stack pile of recent workers active */}
            <div className="flex items-center gap-3 pt-2">
              <div className="flex -space-x-2.5">
                <img src="https://images.unsplash.com/photo-1540569014015-19a7be504e3a?w=100" className="h-7 w-7 rounded-full border border-white object-cover" />
                <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=100" className="h-7 w-7 rounded-full border border-white object-cover" />
                <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100" className="h-7 w-7 rounded-full border border-white object-cover" />
                <div className="h-7 w-7 rounded-full bg-slate-200 border border-white flex items-center justify-center text-[9px] font-bold text-text-secondary">+18</div>
              </div>
              <span className="text-xs font-bold text-text-secondary">More jobs uploaded recently</span>
            </div>
          </div>

          {/* Right Panel: Interactive AI Match Demo Simulator (7 cols) */}
          <div className="lg:col-span-7 bg-slate-50 dark:bg-slate-950 border border-border/10 rounded-3xl p-6 lg:p-8 space-y-6">
            <div>
              <h3 className="text-lg font-display font-black tracking-tight">AI Match Demo</h3>
              <p className="text-xs text-text-secondary font-semibold uppercase tracking-wider mt-0.5">Adjust your preferences to see match score change</p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-text-secondary">
                    ...
                    <span className="text-text-primary">₹{sliderBudget.toLocaleString()}</span>
                  </div>
                  <input 
                    type="range" 
                    min={5000} 
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-text-secondary">
                    ...
                    <span className="text-text-primary">{sliderDistance} km</span>
                  </div>
                  <input 
                    type="range" 
                    min={1} 
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-text-secondary">
                    ...
                    <span className="text-text-primary">{sliderExperience} years</span>
                  </div>
                  <input 
                    type="range" 
                    min={0} 
                    max={10} 
                    step={1}
                    value={sliderExperience} 
                    onChange={(e) => setSliderExperience(parseInt(e.target.value))}
                    className="w-full accent-primary h-1 bg-slate-200 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-bold text-text-secondary">
                    ...
                    <span className="text-text-primary">
                      {sliderAvailability === 4 && 'Full Time'}
                      {sliderAvailability === 3 && 'Flexible'}
                      {sliderAvailability === 2 && 'Part Time'}
                      {sliderAvailability === 1 && 'Any Time'}
                    </span>
                  </div>
                  <input 
                    type="range" 
                    ...
                    onChange={(e) => setSliderAvailability(parseInt(e.target.value))}
                    className="w-full accent-primary h-1 bg-slate-200 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
              </div>

              {/* Matching Circle Visualizer */}
              <div className="flex flex-col items-center justify-center space-y-4">
                {/* SVG Compatibility Gauge Meter */}
                <div className="relative h-36 w-36 flex items-center justify-center">
                  <svg className="h-full w-full transform -rotate-90">
                    ...
                    <motion.circle 
                      ...
                      strokeDashoffset={376.8 - (376.8 * matchScore) / 100}
                  ...
                  <div className="absolute text-center">
                    <div className="text-3xl font-display font-black leading-none">{matchScore}%</div>
                    <span className="text-[9px] font-extrabold uppercase text-emerald-600 mt-1 block">Great Match</span>
                  </div>
                </div>

                {/* Match sub-criteria breakdown checkmarks list */}
                <div className="w-full space-y-2 text-xs font-bold pl-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      <Check className="h-3.5 w-3.5 text-emerald-600" /> Location Match
                    </div>
                    <span className="text-emerald-600">{locPct}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      <Check className="h-3.5 w-3.5 text-emerald-600" /> Budget Match
                    </div>
                    <span className="text-emerald-600">{budPct}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      <Check className="h-3.5 w-3.5 text-emerald-600" /> Experience Match
                    </div>
                    <span className="text-emerald-600">{expPct}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                      <Check className="h-3.5 w-3.5 text-emerald-600" /> Availability Match
                    </div>
                    <span className="text-emerald-600">{availPct}%</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <Button 
              variant="primary" 
              className="w-full mt-4 bg-primary text-xs font-bold uppercase tracking-wider py-3.5 rounded-xl text-white"
              onClick={() => navigate('/auth')}
            >
              View Matched Pros
            </Button>
